    ...
    for (const row of json) {
      ...
      phase.items.push({
        ...
        reference: row.reference?.trim(),
          ...
          // Import main checklist
          await this.importPlane(plane, { planeId, phases });
          this.setImportSummary(`Imported "${name}" main checklist with ${phases.length} phase(s)`);

          // Import type-based variants
          for (const [typeCategory, variantChecklist] of categoryPhaseMaps) {
            if (typeCategory === MAIN_KEY) continue;
            const category = typeCategory || 'standard';
            await this.addCategory(planeId, category, variantChecklist);
            this.setImportSummary(prev => prev + ` Imported variant "${category}"`);
          }

        } catch (error) {
          console.error('Error:', error);
          this.setImportSummary('Error processing JSON');
        }
      };
