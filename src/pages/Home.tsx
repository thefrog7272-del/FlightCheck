...
listAllSharedChecklists()]);
  ...
  const handleJsonImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // ---- Parser ----
    const KNOWN_CATEGORIES = new Set(['normal', 'abnormal', 'reference', 'emergency']);

    function normalizeCategory(value?: string | null): string | undefined {
      if (value === null || value === undefined) return undefined;
      const t = String(value).toLowerCase().trim();
      return KNOWN_CATEGORIES.has(t) ? t : undefined;
    }

    const parseAircraftJson = (json: any): AircraftJson => {
      if (!json || typeof json !== 'object') {
        throw new Error('Invalid JSON: root must be an object');
      }
      const { aircraft, nickname, checklist } = json;
      if (typeof aircraft !== 'string' || typeof nickname !== 'string' || !Array.isArray(checklist)) {
        throw new Error('Invalid JSON: required fields missing or wrong type');
      }

      return {
        aircraft,
        nickname,
        checklist: checklist.map((section: any, idx: number) => {
          if (!section || typeof section !== 'object' || !Array.isArray(section.items)) {
            throw new Error(`Invalid checklist section at index ${idx}`);
          }
          return {
            title: typeof section.title === 'string' ? section.title : `Section ${idx}`,
            items: section.items.map((item: any, i: number) => {
              if (!item || typeof item !== 'object') {
                throw new Error(`Invalid item at section ${idx}, item index ${i}`);
              }
              if (typeof item.callout !== 'string' || typeof item.response !== 'string') {
                throw new Error(`Missing callout or response at section ${idx}, item index ${i}`);
              }

              const rawCategory = typeof item.type === 'string' ? item.type : (typeof item.category === 'string' ? item.category : undefined);
              const category = normalizeCategory(rawCategory);

              return {
                pause: Array.isArray(item.pause) && item.pause.length === 2 && typeof item.pause[0] === 'number'
                  ? [item.pause[0], String(item.pause[1])]
                  : undefined,
                callout: item.callout,
                response: item.response,
                type: category, // normalized category or undefined
                ...Object.fromEntries(Object.entries(item).filter(([k]) => !['pause', 'callout', 'response', 'type', 'category'].includes(k))),
              };
            }),
          };
        }),
    ...
    // ---- Domain builders (example) ----
    const buildPlaneFromAircraft = (nickname: string, aircraft: string) => ({
    ...
    const handleJsonImport = async (filePath: string) => {
      try {
        ...
        const data = parseAircraftJson(json);

        const plane = buildPlaneFromAircraft(data.nickname, data.aircraft);
        const MAIN_KEY = '__main__';
        const categoryPhaseMaps = new Map<string, Map<string, { id: string; title: string; items: any[] }>>();

        for (const section of data.checklist) {
          for (const row of section.items) {
            const phaseTitle = row.pause?.[1] || row.callout;
            const itemLabel = row.response;
            if (!phaseTitle || !itemLabel) continue;

            const category = row.type; // normalized: 'normal' | 'abnormal' | 'reference' | 'emergency' | undefined
            const isMain = !category; // undefined/null => main checklist
            const mapKey = isMain ? MAIN_KEY : category;

            if (mapKey === undefined) continue; // safety; should not happen
            ...
            phase.items.push({
              id: `${phase.id}-${phase.items.length}`,
              label: itemLabel,
              expectedState: row.response || undefined,
              reference: row.pause ? String(row.pause[0]) : undefined,
            });
          }
        }
        ...
        const phases = Array.from(mainPhasesMap.values());
        const totalItems = phases.reduce((sum, p) => sum + p.items.length, 0);
        const checklist = { planeId: plane.id, phases };

        const jsonCategories: Record<string, { planeId: string; phases: typeof phases }> = {};
        for (const [key, phasesMap] of categoryPhaseMaps) {
          if (key === MAIN_KEY) continue;
          jsonCategories[key] = {
            planeId: plane.id,
        ...
        // Example outputs / next steps:
        console.log('Plane:', plane);
        console.log('Checklist:', checklist);
        console.log('Categories:', jsonCategories);

        // TODO: call your domain helpers:
        // await importPlane(plane, checklist);
        // for (const [categoryName, categoryChecklist] of Object.entries(jsonCategories)) {
        //   await addCategory(plane.id, categoryName, categoryChecklist);
        // }

        return { plane, checklist, categories: jsonCategories };
      } catch (error: any) {
    ...
    // Example:
    // handleJsonImport(path.join(__dirname, 'my-checklist.json'));
