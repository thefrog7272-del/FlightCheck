...
const handleImport = async () => {
  try {
    ...
    // Use importPlane instead of addPlane directly so admin path is handled too
    await importPlane(plane, checklist);

    const categoryKeys = Object.keys(variants);
    for (const categoryName of categoryKeys) {
      addCategory(plane.id, categoryName, variants[categoryName]);
    }

    const totalCategoryItems = categoryKeys.reduce(
      (sum, cat) => sum + (variants[cat]?.phases.reduce((s, p) => s + p.items.length, 0) || 0), 0
    );
